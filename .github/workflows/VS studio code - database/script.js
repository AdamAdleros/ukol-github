...
function endGame() {
  ...
  // Display the game over screen with the current score
...
function submitScore(nickname, score) {
  ...
  .then(data => {
      if (data.rank > 10) {
          alert(`Your score is ranked at position: ${data.rank}`);
      } else {
          displayTopScores();
      }
  })
...
function displayTopScores() {
  fetch('get_scores.php')
      .then(response => response.json())
      .then(scores => {
          let scoreList = "<h3>Top 10 High Scores</h3><ul>";
          scores.forEach((score) => {
              scoreList += `<li>${score.nickname}: ${score.score}</li>`;
          });
          scoreList += "</ul>";
          document.getElementById("high-score-display").innerHTML = scoreList;
      })
      .catch(error => console.error('Error:', error));
}